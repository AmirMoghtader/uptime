...
/**
 * فاصلهٔ اسمی بین دو بررسی (دقیقه).
 *
 * هر اجرای Workflow پنج بررسی با فاصلهٔ سه دقیقه می‌کند، و خودِ اجراها هر
 * ۱۵ دقیقه شروع می‌شوند — البته cron گیت‌هاب بدقول است و گاهی دیرتر. پس
 * فاصلهٔ واقعی بین دو بررسی بین ۳ تا حدود ۱۰ دقیقه نوسان دارد و ۵ عددِ
 * میانه‌ی معقولی است.
 *
 * این عدد فقط دو جا اثر دارد: آستانه‌ی «شکاف داده» (سه برابرش) و جمله‌ای که
 * روی پنل نوشته می‌شود.
 */
export const CHECK_EVERY_MIN = 5;
